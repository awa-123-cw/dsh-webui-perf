			...
			function deepClean() {
				...
				try {
					fetch("/webui-perf/clean-deep", { method: "POST", cache: "no-store" })
						...
						.then(function (data) {
							var parts = [];
							if (data && data.result && data.result.steps) {
								var s = data.result.steps;
								if (s.emptyWorkingSets === "ok") parts.push("全进程工作集 ✓");
								if (s.purgeStandby === "ok") parts.push("standby ✓");
								if (data.result.freedGB > 0.05) parts.push("可用 +" + data.result.freedGB + " GB");
								if (data.usedUac && s.emptyWorkingSets !== "ok") parts.push("UAC 未授权或失败");
							} else if (data && data.error) {
								parts.push("失败: " + String(data.error).slice(0, 40));
							}
							finish(parts.length > 0 ? parts.join(" · ") : (data && data.result ? "完成（可用内存未明显变化）" : "无响应"));
						})
