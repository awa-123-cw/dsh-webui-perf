	...
	factory: (require) => {
		...
		function MemoryGauge(props) {
			...
			function deepClean() {
				...
				// 第二次点击：PCL 同款全系统深度清理（host 弹 UAC 提权执行）。
				// 不刷新页面——清理的是系统内存，页面自身换页无碍；刷新反而
				// 会重载会话内容把内存吃回。
				if (busyRef.current) return;
				busyRef.current = true;
				setBusy(true);
				...
				var finish = function (label) {
					...
					setBusy(false);
					setFreed(label);
					window.setTimeout(function () { setFreed(null); }, 8000);
					try {
						fetch("/webui-perf/memory", { cache: "no-store" })
							.then(function (r) { return r.ok ? r.json() : null; })
							.then(function (data) {
				...
				try {
					fetch("/webui-perf/clean-deep", { method: "POST", cache: "no-store" })
						...
						.then(function (data) {
							...
							if (data && data.result) {
								var r = data.result;
								if (typeof r.cleared === "number" && r.cleared > 0) {
									parts.push("清空 " + r.cleared + " 进程工作集");
									if (typeof r.freedWSGB === "number" && r.freedWSGB > 0.05) parts.push("释放 " + r.freedWSGB + " GB");
								}
								if (typeof r.freedGB === "number" && r.freedGB > 0.05) parts.push("可用 +" + r.freedGB + " GB");
								if (r.steps && r.steps.purgeStandby === "ok") parts.push("standby ✓");
							} else if (data && data.error) {
								parts.push("失败: " + String(data.error).slice(0, 40));
							}
							finish(parts.length > 0 ? parts.join(" · ") : (data && data.result ? "完成（无明显变化）" : "无响应"));
						})
						.catch(function () { finish("清理请求失败"); });
				} catch (e) {
					finish("清理请求失败");
				}
				// 兜底：UAC 授权可能较久，60 秒后恢复按钮
				window.setTimeout(function () { finish(beforeFree !== null ? "超时（UAC 未确认？）" : "超时"); }, 60000);
			}
		...
		function apply(ctx) {
			...
			var css = "" +
				...
				".dshWebuiPerfMem{display:inline-flex;align-items:center;gap:6px;height:28px;padding:0 4px 0 10px;border:1px solid var(--dsw-alias-border-l2);border-radius:14px;background:var(--dsw-alias-bg-module-platform);flex:none}" +
				...
				".dshWebuiPerfMemBtn{flex:none;width:24px;height:24px;border:1px solid var(--dsw-alias-border-l2);border-radius:12px;background:var(--dsw-alias-bg-module-platform);cursor:pointer;font-size:13px;line-height:22px;padding:0}" +
				".dshWebuiPerfMemBtn:hover{background:var(--dsw-alias-interactive-bg-hover);border-color:var(--dsw-static-deepseek-500,var(--dsw-alias-state-ok-primary))}" +
