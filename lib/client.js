...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		var en = {
			...
			"mem.tip": "System available / total memory (host os.freemem). 🧹 = clear render caches + GC; ♻️ = deep clean (reload page, session state auto-restores)",
		...
		var zh = {
			...
			"perf.patchMissing": "官方包补丁缺失：dsh 升级可能覆盖了补丁。请重跑 patches/apply-runtime-patches.mjs 后重启 dsh，开关才会生效。",
			"perf.patchMissingBadge": "补丁缺失",
			"mem.tip": "系统可用/总内存（host os.freemem）· 🧹 轻清理（清缓存+GC）· ♻️ 深度清理（刷新页面，会话状态自动恢复）",
			"mem.clean": "轻清理：清空渲染缓存并触发垃圾回收",
			"mem.deep": "深度清理：刷新页面释放进程内存（会话状态已持久化，自动恢复）",
			"mem.deepConfirm": "再点一次确认刷新",
			"mem.reloading": "刷新中…",
			"mem.freed": "已释放 {mb} MB",
		...
		function MemoryGauge(props) {
			...
			function lightClean() {
				...
				var before = (typeof performance !== "undefined" && performance.memory) ? performance.memory.usedJSHeapSize : 0;
				var beforeFree = sys !== null ? sys.free : null;
				window.setTimeout(function () {
					var after = (typeof performance !== "undefined" && performance.memory) ? performance.memory.usedJSHeapSize : 0;
					var heapFreed = before > 0 ? Math.max(0, Math.round((before - after) / 1048576 * 10) / 10) : 0;
					busyRef.current = false;
					setBusy(false);
					// PCL 口径：系统可用内存回升量（host os.freemem 前后对比）
					if (beforeFree !== null) {
						try {
							fetch("/webui-perf/memory", { cache: "no-store" })
								.then(function (r) { return r.ok ? r.json() : null; })
								.then(function (data) {
									if (data && typeof data.free === "number") {
										var freedGB = (data.free - beforeFree) / 1073741824;
										setFreed(freedGB > 0.05
											? "可用 +" + freedGB.toFixed(1) + " GB"
											: (heapFreed > 0 ? t("mem.freed", { mb: heapFreed }) : "已清理 ✓"));
										setSys({ total: data.total, free: data.free });
									} else {
										setFreed(heapFreed > 0 ? t("mem.freed", { mb: heapFreed }) : "已清理 ✓");
									}
								})
								.catch(function () {
									setFreed(heapFreed > 0 ? t("mem.freed", { mb: heapFreed }) : "已清理 ✓");
					...
					window.setTimeout(function () { setFreed(null); }, 5000);
			...
			function deepClean() {
				...
				// 第二次点击：先轻清理，再刷新页面（EmptyWorkingSet 的 WebUI 等价物）
				setReloading(true);
