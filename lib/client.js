		...
		function MemoryGauge(props) {
			...
			if (sys !== null) {
				...
				text = freed !== null
					...
					: "可用 " + freeGB.toFixed(1) + " / " + totalGB.toFixed(1) + " GB";
			} else {
				text = freed !== null
					? t("mem.freed", { mb: freed })
					: (heap.used / 1048576).toFixed(0) + " / " + Math.round(heap.limit / 1048576) + " MB";
			}
			...
			function lightClean() {
				...
				window.setTimeout(function () {
					...
					setBusy(false);
					setFreed(before > 0 ? Math.max(0, Math.round((before - after) / 1048576 * 10) / 10) : 0);
