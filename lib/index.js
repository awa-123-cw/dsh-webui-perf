...
const CLEAN_SCRIPT_DEEP_UAC = CLEAN_SCRIPT_DEEP.replace(
  '[PSCustomObject]@{ beforeFreeKB = $before; afterFreeKB = $after; freedGB = [math]::Round(($after - $before) / 1048576, 2); admin = $admin; privProfile = $privProfile; privQuota = $privQuota; steps = $steps } | ConvertTo-Json -Compress -Depth 3',
  `[PSCustomObject]@{ beforeFreeKB = $before; afterFreeKB = $after; freedGB = [math]::Round(($after - $before) / 1048576, 2); admin = $admin; privProfile = $privProfile; privQuota = $privQuota; steps = $steps; elevated = ([Security.Principal.WindowsIdentity]::GetCurrent().Owner.Value -eq 'S-1-5-32-544') } | ConvertTo-Json -Compress -Depth 3 | Set-Content -Path $OutFile -Encoding UTF8`,
)
...
function runCleanDeep(res) {
  ...
  try {
    execFileSync('powershell.exe', ['-NoProfile', '-NonInteractive', '-ExecutionPolicy', 'Bypass', '-File', ps1, '-OutFile', outFile], {
      timeout: 60000,
      windowsHide: true,
    })
    dbg('direct run: exit ok')
  } catch (e) {
    dbg(`direct run: failed ${String(e.message ?? e).slice(0, 150)}`)
  }
