...
$admin = ([Security.Principal.WindowsPrincipal][Security.Principal.WindowsIdentity]::GetCurrent()).IsInRole([Security.Principal.WindowsBuiltInRole]::Administrator)
$before = (Get-CimInstance Win32_OperatingSystem).FreePhysicalMemory
$privProfile = $false; $privQuota = $false; $privLastErr = ''
if ($admin) {
  $privProfile = Enable-Privilege 'SeProfileSingleProcessPrivilege'
  $privLastErr = ('0x{0:X8}' -f ([Runtime.InteropServices.Marshal]::GetLastWin32Error() -band 0xFFFFFFFF))
  $privQuota = Enable-Privilege 'SeIncreaseQuotaPrivilege'
  if ($privLastErr -eq '0x00000000') { $privLastErr = ('0x{0:X8}' -f ([Runtime.InteropServices.Marshal]::GetLastWin32Error() -band 0xFFFFFFFF)) }
}
$privDump = (whoami /priv 2>&1 | Out-String).Trim()
...
# 核心：逐进程 EmptyWorkingSet —— 清空所有可打开进程的工作集（PCL 主线
# 对半砍的真正来源；无需 SYSTEM 级特权，Windows 11 24H2 上
# NtSetSystemInformation(MemoryEmptyWorkingSets) 已被收紧为 SYSTEM only）
$freedWS = 0L; $cleared = 0
Get-Process -ErrorAction SilentlyContinue | ForEach-Object {
  $h = [MemUtil2]::OpenProcess(0x500, $false, $_.Id)
  if ($h -ne [IntPtr]::Zero) {
    if ([MemUtil2]::EmptyWorkingSet($h)) { $freedWS += $_.WorkingSet64; $cleared++ }
    [MemUtil2]::CloseHandle($h) | Out-Null
  }
}
$steps['perProcess'] = "ok($cleared)"
# 系统级补充（24H2 上多为拒绝，尽力而为）
...
$after = (Get-CimInstance Win32_OperatingSystem).FreePhysicalMemory
$json = [PSCustomObject]@{ beforeFreeKB = $before; afterFreeKB = $after; freedGB = [math]::Round(($after - $before) / 1048576, 2); freedWSGB = [math]::Round($freedWS / 1073741824, 2); cleared = $cleared; admin = $admin; privProfile = $privProfile; privQuota = $privQuota; steps = $steps; elevated = ([Security.Principal.WindowsIdentity]::GetCurrent().Owner.Value -eq 'S-1-5-32-544') } | ConvertTo-Json -Compress -Depth 3
if ($OutFile) { $json | Set-Content -Path $OutFile -Encoding UTF8 }
Write-Output $json
`

/**
 * 深度清理脚本已合并为单版本（CLEAN_SCRIPT_DEEP）：
 * param([string]$OutFile) 可选；direct 与 UAC 提权共用一份脚本，
 * 结果同时写文件（若给 OutFile）和 stdout（双保险）。
 */
...
/**
 * 深度清理：逐进程 EmptyWorkingSet（清所有可打开进程工作集，PCL 对半砍核心）
 * + standby/flush 尽力 + UAC 提权（可打开更多进程）。direct 先跑当前令牌，
 * 若 perProcess 成功数偏低或无结果则 UAC 提权重试。结果 stdout/文件双通道。
...
async function runCleanDeep(res) {
  ...
  writeFileSync(ps1, CLEAN_SCRIPT_DEEP)
  ...
  // stdout 双保险：脚本 Write-Output 会把 JSON 打到 stdout，文件失败也能拿结果
  ...
  // 逐进程方案普通权限即可清同用户进程；UAC 提权后能开更多进程（含 SYSTEM 服务），
  // 清空数明显更多时才算提权有价值。direct 已清到多数进程时不再打扰用户。
  const directCleared = result?.cleared ?? 0
  if (result === null || directCleared < 20) {
